...
bool runCommand(const std::string& command, const std::string& statsFile, std::map<std::string, long long>& passStats, std::map<std::string, long long>& analysisStats) {
    const std::string ERR_LOG = "command_error.log";
    ...
    if (errFile.is_open()) {
        ...
        while (std::getline(errFile, line)) {
            // New, more robust parsing for `opt -stats`
            if (line.find("instcount") != std::string::npos && line.find("instructions") != std::string::npos) {
                std::stringstream ss(line);
                long long val;
                if (ss >> val) analysisStats["Instruction Count"] = val;
            } else if (line.find("basicaa") != std::string::npos && line.find("Number of basic blocks") != std::string::npos) {
                std::stringstream ss(line);
                long long val;
                if (ss >> val) analysisStats["Basic Block Count"] = val;
            } else if (line.find("[BogusInsert] inserted ") != std::string::npos) {
                long long inserts = 0;
...
ObfuscationResult performObfuscation(const std::string& inputSourceFile, const std::string& outputExecutableName, bool keepIntermediateFiles, ObfuscationConfig& config) {
    ...
    if (!runCommand(CLANG + " -S -emit-llvm -O0 " + inputSourceFile + " -o " + currentIRFile, "", result.stats, result.initialAnalysis)) return result;
    ...
    int totalSteps = (config.stringObfuscation ? config.stringObfCycles : 0) + (config.bogusControlFlow ? config.bogusControlFlowCycles : 0) + (config.controlFlowFlattening ? config.flatteningCycles : 0);
    ...
    if (!keepIntermediateFiles) {
        ...
        runCommand("rm -f stats_*.json command_error.log", "", result.stats, result.finalAnalysis);
    }
